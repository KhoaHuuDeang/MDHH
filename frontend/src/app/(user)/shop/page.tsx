...
export default function ShopPage() {
  ...
  const addToCart = async (souvenirId: string) => {
    try {
      ...
      if (res.data.status === 200) {
        alert('Added to cart!');
        // Trigger cart header refresh by dispatching custom event
  ...
  if (loading) return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#F0F8F2]">
        ...
        <span className="font-medium text-[#386641]">Finding best souvenirs...</span>
    </div>
  );
    ...
    <div className="min-h-screen bg-gray-100 py-8">
      <div className="max-w-[1200px] mx-auto px-4">
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm mb-6 flex flex-col md:flex-row justify-between items-center border-l-4 border-[#386641]">
            <div>
                <h1 className="text-2xl font-bold text-[#386641]">Souvenir Shop</h1>
                <p className="text-gray-500 text-sm mt-1">Discover unique items for your collection</p>
            </div>
            {/* Filter decoration (Visual only) */}
            <div className="mt-4 md:mt-0 flex gap-2 text-sm">
                <span className="bg-[#386641] text-white px-3 py-1 rounded-full cursor-pointer shadow-sm">All</span>
                <span className="bg-white text-gray-600 px-3 py-1 rounded-full border border-gray-200 cursor-pointer hover:border-[#386641] hover:text-[#386641] transition-colors">Popular</span>
                <span className="bg-white text-gray-600 px-3 py-1 rounded-full border border-gray-200 cursor-pointer hover:border-[#386641] hover:text-[#386641] transition-colors">Newest</span>
            </div>
        </div>
          ...
          {souvenirs.map((item) => (
            ...
            >
              ...
              <div className="relative w-full aspect-square overflow-hidden bg-gray-100">
                ...
                {item.stock === 0 && (
                    <div className="absolute inset-0 bg-black/50 flex items-center justify-center z-10">
                        <span className="bg-gray-800 text-white text-xs px-3 py-1 rounded-full font-bold uppercase tracking-wider">Sold Out</span>
                    </div>
                )}
              </div>
              ...
              <div className="p-3 flex flex-col flex-1">
                ...
                {/* Tags/Description truncation (Visual touch) */}
                ...
                <div className="mt-auto flex items-end justify-between gap-2">
                    ...
                    <span className="text-[10px] text-gray-500 mb-1">Stock: {item.stock}</span>
                </div>
                
                {/* Shopee-like Hover Button or Static Button */}
                <button
                  onClick={(e) => {
                    // Prevent card click if we ever added navigation to card
                    e.stopPropagation();
                    addToCart(item.id);
                  }}
